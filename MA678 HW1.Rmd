...
```{r}
plot.new()
text(x = 10, y = 50, label = paste("y = ", round(coef(lm_model)[1], 2), " + ", round(coef(lm_model)[2], 2), "x"), pos = 4)
```
...
lm_model <- lm(y ~ x)
stan_glm_model <- stan_glm(y ~ x, data = data.frame(x = x, y = y))
coef(lm_model)
coef(stan_glm_model)
summary(lm_model)
...
lines(x, predict(fit_stan_glm, newdata = data.frame(x = sort(x))), col = "blue")
```
